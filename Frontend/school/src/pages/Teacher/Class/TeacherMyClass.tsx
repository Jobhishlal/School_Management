...
const TeacherMyClass: React.FC = () => {
    ...
    return (
        <div className={`p-6 max-w-7xl mx-auto space-y-8 min-h-screen ${pageBg} transition-colors duration-300`}>
            {/* Header Section */}
            <div className={`${cardBg} p-6 rounded-2xl shadow-sm border ${borderColor} flex flex-col md:flex-row justify-between items-start md:items-center gap-4`}>
                <div>
                    <h1 className={`text-3xl font-bold mb-2 ${isDark ? "text-white" : "text-slate-900"}`}>
                        {classData.classInfo.className} - {classData.classInfo.division}
                    </h1>
                    <p className={`${isDark ? "text-slate-400" : "text-slate-500"}`}>View and manage details for this class</p>
                </div>
                <div className="flex flex-col items-end">
                    <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>Class Teacher</p>
                    <p className={`font-semibold text-lg ${isDark ? "text-white" : "text-slate-800"}`}>
                        {classData.teacherName || "Unknown"}
                    </p>
                </div>
            </div>

            {/* Overview Stats (Only Total Students remaining) */}
            <div className={`${cardBg} p-6 rounded-xl shadow-sm border ${borderColor} w-full md:w-1/3`}>
                <p className="text-sm text-slate-500 mb-1">Total Students</p>
                <p className={`text-2xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{classData.totalStudents}</p>
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                <div className={`${cardBg} rounded-2xl shadow-sm border ${borderColor} overflow-hidden`}>
                    <div className={`p-6 border-b ${borderColor} flex justify-between items-center flex-wrap gap-4`}>
                        <h2 className={`text-xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>Student List</h2>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className={`pl-10 pr-4 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-64 border ${isDark
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className={`${cardBg} p-6 rounded-2xl shadow-sm border ${borderColor}`}>
                        <div className="mb-6">
                            <h2 className={`text-lg font-bold mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>Performance Chart</h2>
                            <div className="flex items-center gap-2">
                                <span className={`text-3xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{classData.stats.performance.average}%</span>
                                <span className="text-sm text-green-500 font-medium">Avg Marks</span>
                            </div>
                        </div>
                        <div className="h-64">
                    ...
                    <div className={`${cardBg} p-6 rounded-2xl shadow-sm border ${borderColor}`}>
                        <div className="mb-6">
                            <h2 className={`text-lg font-bold mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>Attendance Graph</h2>
                            <div className="flex items-center gap-2">
                                <span className={`text-3xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{classData.stats.attendance.percentage}%</span>
                                <span className="text-sm text-green-500 font-medium">Class Avg</span>
                            </div>
                        </div>
                        <div className="h-48">
                            <ResponsiveContainer width="100%" height="100%">
                    ...
                    <div className={`${cardBg} p-6 rounded-2xl shadow-sm border ${borderColor}`}>
                        <div className="mb-6">
                            <h2 className={`text-lg font-bold mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>Avg. Marks vs School</h2>
                            <div className="flex items-center gap-2">
                                <span className={`text-3xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{classData.stats.performance.average}%</span>
                                <span className="text-sm text-green-500 font-medium">Class Avg</span>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between text-xs text-slate-500 mb-1">
                                    <span>Class Avg</span>
                                    <span>{classData.stats.performance.average}%</span>
                                </div>
                                <div className={`w-full rounded-full h-8 overflow-hidden ${isDark ? "bg-slate-700" : "bg-slate-100"}`}>
                                    <div className="bg-blue-600/20 h-full rounded-r-lg border-r-4 border-blue-500 relative" style={{ width: `${classData.stats.performance.average}%` }}></div>
                                </div>
                            </div>
                            <div>
                                <div className="flex justify-between text-xs text-slate-500 mb-1">
                                    <span>School Average</span>
                                    <span>{classData.stats.schoolAverage}%</span>
                                </div>
                                <div className={`w-full rounded-full h-8 overflow-hidden ${isDark ? "bg-slate-700" : "bg-slate-100"}`}>
                                    <div className="bg-slate-400/20 h-full rounded-r-lg border-r-4 border-slate-400 relative" style={{ width: `${classData.stats.schoolAverage}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
