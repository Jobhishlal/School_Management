...
const ParentSidebar: React.FC<ParentSidebarProps> = ({ children }) => {
    ...
    >
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <h1 className={`text-lg font-bold ${textPrimary}`}>Parent Portal</h1>
        </div>
        <button
          onClick={toggleTheme}
      ...
      <div className="flex h-full">
        {/* ---------- SIDEBAR ---------- */}
        <div
          className={`fixed lg:relative top-0 left-0 z-50 h-full w-72 transform transition-all duration-500 ease-out
        ...
        <div className={`flex-1 flex flex-col h-full ${isDark ? "bg-[#121A21]" : "bg-slate-50"}`}>
          ...
          >
            <div className={`px-6 py-3 rounded-2xl ${cardBg} border ${headerBorder} shadow-sm`}>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                {showProfileDropdown && (
                  <div
                    className={`absolute right-0 top-full mt-2 w-48 ${cardBg} border ${headerBorder} rounded-xl shadow-xl z-50 py-2`}
                  >
                    <Link
                      to="/parent/profile"
                      className={`flex items-center gap-3 px-4 py-3 ${hoverBg} ${textSecondary} hover:text-white`}
                    >
                      <Settings size={16} />
                    ...
                    <button
                      onClick={handleLogout}
                      className={`w-full flex items-center gap-3 px-4 py-3 ${hoverBg} text-red-500 hover:text-red-600`}
                    >
                      <LogOut size={16} />
