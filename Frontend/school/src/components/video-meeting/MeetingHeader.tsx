...
const MeetingHeader: React.FC<MeetingHeaderProps> = ({
    ...
    return (
        <header className={`px-6 py-4 flex items-center justify-between border-b backdrop-blur-md relative z-50 ${isDark ? 'bg-gray-900/80 border-gray-800' : 'bg-white/80 border-gray-200'}`}>
            ...
            {waitingList.length > 0 && (
                <div className="relative">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 ${showLobby ? 'bg-blue-700 ring-2 ring-blue-500/50' : 'bg-blue-600 hover:bg-blue-700'} text-white rounded-full text-sm font-medium transition-all shadow-lg shadow-blue-600/20 active:scale-95`}
                    >
                        <span>Lobby</span>
                    ...
                    {showLobby && (
                        <div className={`absolute top-full right-0 mt-2 w-80 rounded-2xl shadow-2xl border p-3 z-50 animate-in fade-in slide-in-from-top-2 ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`}>
